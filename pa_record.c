                          ...
                          PaStreamCallbackFlags statusFlags, void *dataFile)
{
...
#if COMPUTE_XCORR
    shintPubAttrCheck(1, NULL, (wxSpeaCalFrame *) userDataFile->mainThread, pubBufFileID);
    ...
    else
    {
        ...
        shintPubAttrBitToggle(0, pubBufFileID, (wxSpeaCalFrame *) userDataFile->mainThread, userDataFile->devNum);
        gettimeofday(&currTime, NULL);
        TimevalDiff(&timeDiff, &currTime, &userDataFile->pubStartTime, (wxSpeaCalFrame *) dummy);
        if(userDataFile->pubMenuNormTDOA)
        {
            doublePubAttrAssign(3, userDataFile->devNum - FIRST_MIC_ID,
                                (wxSpeaCalFrame *) userDataFile->mainThread,
...
void *pa_record_thread(void *ptr)
{
    ...
    timevalPubStartTimeCheck((wxSpeaCalFrame *) pPAThreadData->mainThread, startTime);

    intPubAttrCheck(6, (wxSpeaCalFrame *) pPAThreadData->mainThread, FramesPerBuffer);
    userDataFile.mainThread = pPAThreadData->mainThread;
    intPubAttrCheck(0, (wxSpeaCalFrame *) pPAThreadData->mainThread,
    ...
    TimevalDiff(&timeDiff, &currTime, &startTime,
                (wxSpeaCalFrame *) pPAThreadData->mainThread);
    ...
    {
        Pa_Sleep(1000);
        intPubAttrCheck(1, (wxSpeaCalFrame *) pPAThreadData->mainThread, PAStreamRun);
        if(!PAStreamRun)
            break;
